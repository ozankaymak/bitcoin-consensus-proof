...
use jmt::{proof::{SparseMerkleProof, UpdateMerkleProof}, KeyHash, OwnedValue, RootHash, Version};
...
pub struct UTXOSetGuest {
    ...
    #[serde(skip)]
    #[borsh(skip)]
    utxo_cache: BTreeMap<KeyOutPoint, UTXO>,
    spent_utxos: BTreeMap<KeyOutPoint, UTXO>,
}

/// A UTXO key represents a specific output from a transaction
#[derive(Serialize, Deserialize, Eq, PartialEq, Clone, Debug, BorshDeserialize, BorshSerialize, Hash, Ord, PartialOrd)]
...
impl UTXO {
    ...
    /// Convert UTXO to bytes for JMT storage
...
impl UTXOSetGuest {
    ...
    /// Get the complete UTXO
    pub fn get_utxo(&self, utxo_key: &KeyOutPoint) -> UTXO {
        // Check cache first
        if let Some(utxo) = self.get_cached_utxo(utxo_key) {
            return utxo;
        }
        
        // In a zkVM context, we need to provide a proof for this UTXO
        // This stub implementation will panic if not in cache
        panic!("UTXO not found in cache: {:?}", utxo_key);
    }
    ...
    pub fn add_transaction_outputs(
        ...
        // from the host, but for now we just store in the cache
    }
    ...
    pub fn verify_inclusion_proof(&self, utxo_key: &KeyOutPoint, utxo: &UTXO, batch_proof: &TransactionElementsBatchProof) -> bool {
        for proof in batch_proof {
            let key_hash = utxo_key.to_key_hash();
            let value = utxo.to_bytes();
            
            proof.proof.verify(self.jmt_root, key_hash, Some(&value)).is_ok()
        }
        true
    }
    
    /// Verify an inclusion proof for a UTXO that doesn't exist
    pub fn verify_non_inclusion_proof(&self, utxo_key: &KeyOutPoint, proof: &UTXOInclusionProof) -> bool {
        let key_hash = utxo_key.to_key_hash();
        proof.proof.verify(self.jmt_root, key_hash, None).is_ok()
    }
    
    /// Verify a range of UTXOs (useful for proving absence of double-spends)
    pub fn verify_range_proof(&self, proof: &UTXOInclusionProof, start_key: &KeyOutPoint, end_key: &KeyOutPoint) -> bool {
        let start_hash = start_key.to_key_hash();
        let end_hash = end_key.to_key_hash();
        
        proof.range_proof.verify(self.jmt_root, start_hash, end_hash).is_ok()
    }
    ...
    pub fn apply_updates(&mut self, updates: &[UTXOUpdate]) -> bool {
        ...
        for update in updates {
            ...
            if update.value.is_some() {
                ...
                if let Ok(root) = update.proof.update_proof.verify_update(
                    new_root,
                    key_hash,
                    Some(&value_bytes),
                ) {
