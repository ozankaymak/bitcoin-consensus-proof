...
use bitcoin_consensus_core::zkvm::{ZKProof, ZkvmGuest};
...
impl MockZkvmHost {
    pub fn new() -> Self {
        // println!("Creating a new MockZkvmHost");
        Self::default()
    }
}
...
impl ZkvmHost for MockZkvmHost {
    ...
    fn prove(&self, _elf: &[u32]) -> ZKProof {
        let data = self.data.lock().unwrap();
        ZKProof {
            method_id: [42; 8],
    ...
    fn add_assumption(&self, _proof: ZKProof) {}
}
